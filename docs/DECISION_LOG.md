...
**Impacto:** `.claude/settings.json` gana `permissions` + hook `PreToolUse`; nuevo `scripts/deny_destructive.py` (~130 líneas, solo stdlib, ~56 ms por invocación) y `tests/test_deny_destructive.py` (39 casos) para que CI proteja el guard. El parseo es por tokens vía `shlex` con `punctuation_chars`, no por subcadenas: trocear el comando por `;` antes de tokenizar rompía los strings citados y hacía que `echo "ojo con git push --force"` se denegara como force-push real.
