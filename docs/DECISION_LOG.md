...
**Decisión:** Solo se publican los valores con ancla confirmada (CHB-1=6.5, CHB-4=11.5, CHB-5=8.9, CHB-7=3.0, CHB-21=5.6). Para CHB-2 y CHB-3 se registra la mejor lectura visual con sufijo `?` (`8.5?` y `4?` respectivamente). Las 22 estaciones restantes y los parámetros adicionales (OD, pH, T, conductividad, nitratos, amoniaco, turbidez) requieren extracción con Docling+GPU en gorgo (`bd memories gorgo-docling-ocr-pipeline`).
**Impacto:** `peblt_crucero_hidroquimico_i_2019.csv` queda en 7 estaciones (sin expansión a 29); CHB-2 y CHB-3 pasan de `?` a `8.5?`/`4?` (uncertain mejorado). La extracción completa se pospone al pipeline gorgo (issue `lo9`).
